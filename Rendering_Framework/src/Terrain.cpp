...
void Terrain::update(GLuint reflection_fbo, GLuint refraction_fbo, vec3 view_point_light_pos2, mat4 view_matrix2, bool activateWater) {
	...
	if (activateWater) {
		...
		glClearBufferfv(GL_COLOR, 0, value_ptr(vec4(1.0, 1.0, 1.0, 1.0)));
		glClear(GL_DEPTH_BUFFER_BIT);
		glUniform4fv(manager->planeHandle, 1, value_ptr(vec4(0.0, -1.0, 0.0, 107.0)));
		for (int i = 0; i < 4; i++) {
			glUniformMatrix4fv(manager->m_modelMatHandle, 1, false, glm::value_ptr(this->m_chunkRotMat[i]));
		...
		glClear(GL_DEPTH_BUFFER_BIT);
		glUniform4fv(manager->planeHandle, 1, value_ptr(vec4(0.0, 1.0, 0.0, -107.0)));
		glUniform3fv(manager->m_pointlightHandle, 1, glm::value_ptr(view_point_light_pos2));
		glUniformMatrix4fv(manager->m_viewMatHandle, 1, false, glm::value_ptr(view_matrix2));
...
Terrain::Terrain(Depth *depth) {
	...
	depth->elevationTex = this->m_elevationTex;
	...
	depth->vToHeightUVMat = this->m_worldToHeightUVMat;
	...
	for (int i = 0; i < 4; i++) {
		...
		//depth->chunkRotMat[i] = this->m_chunkRotMat[i];
	}

	//for (int i = 0; i < NUM_VERTEX * 3; i++) {
	//	depth->vertices.push_back(vertices[i]);
	//}
	//for (int i = 0; i < NUM_INDEX; i++) {
	//	depth->indices.push_back(indices[i]);
	//}

	//depth->terrianNum = NUM_VERTEX;
	//depth->indexCount = NUM_INDEX;
