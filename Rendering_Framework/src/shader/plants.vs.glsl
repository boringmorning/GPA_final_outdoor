...
void main()
{
	vec3 instanced_pos = position + offset;
	vec4 P = view_matrix * vec4(instanced_pos, 1.0);
	...
	gl_Position = vp_matrix * vec4(instanced_pos, 1.0);
	vs_out.texcoord = texcoord;
	vs_out.shadow_coord = shadow_matrix * vec4(instanced_pos, 1.0);

	gl_ClipDistance[0] = dot(vec4(instanced_pos, 1.0), plane);
	vs_out.WV = normalize(instanced_pos);
	vs_out.WN = normal;
}